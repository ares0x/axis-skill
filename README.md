...
本 Skill 已完全重构为 **references 扁平引用**架构的 Agentic Skill。AI 智能体（如 Claude Code 或 claude.ai）读取 `SKILL.md` 的指令后，按需加载 `references/` 下的流程协议文档，通过对话驱动来调度底层计算引擎及在线高考数据接口。

---

## 🛠️ Repository Directory

- `SKILL.md` — Agent 总入口与路由规则、CLI 调用规范。
- `references/` — 流程协议文档（按阶段按需读取）：
  - `profile_protocol.md` — 信息采集三阶段闸门协议（Gate 1-3），含 8 题 Holland 性格测试。
  - `analyze_protocol.md` — 五阶段推理与三官后台激辩协议。
  - `report_protocol.md` — 1+X 生存报告输出格式规范。
  - `snapshot_protocol.md` — 状态快照存档/恢复/列表操作协议。
  - `user_state_tpl.md` — 考生状态模板（v3.0 嵌套 schema）。
- `scripts/` — 底层计算引擎：
  - `runner.py` — CLI 入口与状态管理（支持多命令链式调用）。
  - `evaluator.py` — 志愿评估引擎（支持 4 梯度”冲稳保垫”推荐、大类分流、体检及周期 veto）。
  - `trait_evaluator.py` — Holland RIASEC 测评转换（含 8 题计分 `evaluate_8q`）。
  - `sogou_api.py` — 在线高考数据接口客户端（拉取各省批次线与一分一段位次表）。
  - `gaokao_mapper.py` — 省份归一化与新高考流派科目映射器。
  - `output_generator.py` — 1+X 报告生成器。
  - `fallback_data.py` — 本地 fallback 数据（省控线与位次估算）。
  - `injector.py` — 知识库注入与匹配。
  - `mbti_evaluator.py` — MBTI 性格评估与专业匹配（可选）。
- `data/` — 静态专家决策规则、产业规划、撤销/新增专业名单及省控线数据库。
- `tests/` — 单元测试与集成测试。
...
克隆后在此目录下启动 `claude` 命令行，Claude Code 会通过根目录的 `CLAUDE.md` 与 `SKILL.md` 自动识别并挂载本 Skill 决策引擎。
...
你可以直接在终端中交互式运行 `python3 scripts/runner.py` 或让 AI 智能体（如 Claude Code 或 claude.ai）通过读取 `references/` 协议文档运行以下指令：
...
- `score`: 高考文化总分。若已设置 `province`，系统将自动调用在线高考数据接口转换为预估位次 `rank`。
...
- **命令行工具绑定**：如果您希望 Claude 直接执行本地计算以保证零幻觉，可以提示 Claude 使用 `python3 scripts/runner.py` 作为执行代理。
...
```json
{
  "mcpServers": {
    "sanage-axis-analyzer": {
      "command": "python3",
      "args": ["/absolute/path/to/scripts/runner.py", "--test-mode"]
    }
  }
}
```
...
- 在线高考数据接口（如省控线与一分一段位次表）在本地具有缓存保护机制，数据将被存储在隐藏目录 `.cache/` 下。
...
- 专业 AI 替代率及就业稳定性（AI replacement rates & employment stability）已从代码中解耦，统一配置在独立文件 [data/major_rules.json](https://github.com/ares0x/axis-skill/blob/main/data/major_rules.json) 中。
...
## 👤 关于作者与社区 (About Jace & Communities)

我是 **Jace**，本项目是由我个人独立开发并开源的高考志愿填报决策系统。如果您对本项目有任何建议，或者想探讨 AI 与高考志愿填报的结合，欢迎通过以下渠道与我联系：

*   **GitHub**: [ares0x](https://github.com/ares0x) (欢迎 Star ⭐ 关注本项目)
*   **AI 技术探索与行业观察 (面向开发者与极客)**:
    *   **小红书 / 抖音**: `Sanage Lab` (分享垂直行业 AI Agent 实战案例)
